...
int rowSeconds = 10; //The number of seconds before adding a new row
int sec = MDClock.getElapsedTime().asSeconds(); //Elapsed time
bool Row = false; //function to add a new row
int levelLines = 12, levelColumns = 10;
bool gameOver;

void newRow(int PlayField[12][10])
{
	for(int i=levelLines - 1; i>0; i--)
	{
		for(int j=0; j<levelColumns; i++)
		{
...
int main()
{
	...
	for(i = 0; i<levelLines; i++)
	{
		for(j = 0; j<levelColumns; j++)
		{
		if(i < 3)
			{
	...
	drawDebugLayout(PlayField, JesterLocation.x/50); //draws the jester location as a 1 in the debug console
	...
	while (window.isOpen()) //The Game Window Loop
	{
		while (window.pollEvent(event))
		{
			...
			else if (event.type == Event::KeyPressed) //Reading keyboard keys
			{
				if(ResetGame)
				{
				if (Keyboard::isKeyPressed(Keyboard::Left)) //Enum SFML Input Manager's Code
					{
						if (JesterLocation.x >= 32)
							{
							JesterLocation.x = JesterLocation.x - 50;
							}
						drawDebugLayout(PlayField, JesterLocation.x/50);
					}
					...
					JesterSprite.setPosition(JesterLocation.x, JesterLocation.y);
			if (Keyboard::isKeyPressed(Keyboard::Return)) //This Is NOT The ASCII Code of The Key Pressed, But The "Enum" SFML Input Manager's Code...
				{
				ResetGame = true;
				StartGame = true;
				}
                   ...
                   // newRow(PlayField);
					window.clear();
					sec = (int)MDClock.getElapsedTime().asSeconds();
					...
					//Add a new row
					if(sec % rowSeconds == 0 && !Row) {
					if(!gameOver == 0) {
						newRow(PlayField);
					}
					Row = true;
					} else if (sec % rowSeconds == rowSeconds - 1) {
						MDClock.restart();
						Row = false;
					}
